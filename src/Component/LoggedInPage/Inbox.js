...
export default function Inbox() {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://react-tutorial-ea45d-default-rtdb.firebaseio.com/user/inbox/${changedEmail}.json`
        );
  ...
  const hideBtnHandler = async (index, emailId) => {
    ...
    try {
      const response = await fetch(
        `https://react-tutorial-ea45d-default-rtdb.firebaseio.com/user/inbox/${changedEmail}/${emailId}.json`,
        {
          method: "PUT",
          body: JSON.stringify({
            id: emailId,
