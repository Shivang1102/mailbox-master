...
export default function Inbox() {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://react-tutorial-ea45d-default-rtdb.firebaseio.com/user/sent/${changedEmail}.json`,
  ...
  const dltbtnhandler = async (emailId) => {
    ...
    try {
      const response = await fetch(
        `https://react-tutorial-ea45d-default-rtdb.firebaseio.com/user/sent/${changedEmail}/${emailId}.json`,
        {
          method: "DELETE",
